...
base_script = """(function() {
    const config = {
        vibe: '[VIBE]',
        color: '[COLOR]',
        audioUrl: 'https://raw.githubusercontent.com/trendycahedecor25/Vibezmode/main/music/[VIBE].mp3',
...
# Generate analytics-enabled scripts
for vibe, color in vibes.items():
    content = base_script.replace('[VIBE]', vibe).replace('[COLOR]', color)
    with open(f'v1/analytics-{vibe}.js', 'w') as f:
        f.write(content)

print("Generated analytics-enabled scripts for all vibes.")
